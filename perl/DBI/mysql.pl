...
my $dbuser = "root";
my $dbpass = "Iloveyoubaby2";
my $dbh = DBI->connect($database, $dbuser, $dbpass);
...
my $dbh = DBI->connect($database, $dbuser, $dbpass, { RaiseError => 1, AutoCommit => 0 }) 
...
# 删除数据表
my $drop = "DROP TABLE IF EXISTS employees";
...
# 查询
my $select = "select * from employees limit 10";
